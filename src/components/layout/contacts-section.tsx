...
const contacts = {
  ru: {
    ...
    instituteLabel: "Институт инноваций в технологиях бизнеса",
    addressLabel: "Адрес",
    phoneLabel: "Телефоны",
    emailLabel: "Почта",
...
export function ContactsSection({ locale, className }: ContactsSectionProps) {
  ...
  return (
    <section className={cn("rounded-[32px] border border-slate-200 bg-slate-50/80 p-6 shadow-sm sm:p-8", className)}>
      <div className="mb-8">
        <p className="text-xs font-semibold uppercase tracking-[0.32em] text-[#C41E3A]">
          Industry Skills
        </p>
      ...
      <div className="mt-6 rounded-[28px] bg-slate-950 px-6 py-7 text-white sm:px-8">
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1.25fr)_minmax(0,1fr)_auto] lg:items-center">
          <div>
            ...
            <p className="mt-3 text-sm text-white/70">{copy.instituteLabel}</p>
          </div>

          <div className="grid gap-5 sm:grid-cols-3 lg:grid-cols-1">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.24em] text-white/45">{copy.addressLabel}</p>
              <p className="mt-2 text-sm font-medium text-white/90">{copy.addressCountry}</p>
              <p className="mt-1 text-sm leading-6 text-white/65">{copy.addressLine}</p>
            </div>
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.24em] text-white/45">{copy.phoneLabel}</p>
              <div className="mt-2 space-y-1 text-sm text-white/90">
                <a className="block transition-colors hover:text-white" href="tel:+78126440126">
                  +7 (812) 644-01-26
                </a>
                <a className="block transition-colors hover:text-white" href="tel:+79812015823">
                  +7 (981) 201-58-23
                </a>
              </div>
            </div>
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.24em] text-white/45">{copy.emailLabel}</p>
              <a className="mt-2 block text-sm text-white/90 transition-colors hover:text-white" href="mailto:industryskills@iitb.ru">
