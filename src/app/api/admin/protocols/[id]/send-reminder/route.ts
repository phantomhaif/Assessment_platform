...
export async function POST(
  ...
  try {
    ...
    const recipients = assignments
      .filter((assignment) => !signedUserIds.has(assignment.user.id))
    ...
    const subject = `Напоминание / Reminder: ${protocol.title}${protocol.titleEn ? ` / ${protocol.titleEn}` : ""}`
    const html = buildReminderEmail({
      eventName: protocol.event.name,
      protocolTitle: protocol.title,
      protocolTitleEn: protocol.titleEn || protocol.title,
      protocolId,
    })
  ...
  return `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 0; background: #f8fafc;">
      ...
      <div style="background: white; padding: 40px; border-radius: 0 0 12px 12px; border: 1px solid #e2e8f0; border-top: none;">
        ...
        <p style="color: #64748b; line-height: 1.6; margin: 0 0 8px 0;">Вам необходимо ознакомиться и подписать протокол:</p>
        ...
        <h2 style="color: #1e293b; font-size: 20px; margin: 24px 0 16px 0;">Protocol signature reminder</h2>
        <p style="color: #64748b; line-height: 1.6; margin: 0 0 8px 0;">Please review and sign the following protocol:</p>
        <div style="background: #f8fafc; border-left: 4px solid #C41E3A; padding: 16px; margin: 0 0 24px 0;">
          <p style="margin: 0; color: #1e293b; font-weight: 600;">${escapeHtml(protocolTitleEn)}</p>
        </div>
